...
def test_find_by_context_types_returns_matching_recommendations(
    mock_settings: Settings,
) -> None:
    repository = RecommendationRepositoryImpl(settings=mock_settings)

    result = repository.find_by_context_types([ContextType.WORK_SCHEDULE])

    assert len(result) == 1
    assert result[0].context_type == ContextType.WORK_SCHEDULE
    assert result[0].response_style == ResponseStyle.PROFESSIONAL
    assert result[0].formality_level == FormalityLevel.HIGH
    assert result[0].detail_level == DetailLevel.MEDIUM


def test_find_by_context_types_returns_empty_list_for_no_matches(
    mock_settings: Settings,
) -> None:
    repository = RecommendationRepositoryImpl(settings=mock_settings)

    result = repository.find_by_context_types([ContextType.AVAILABILITY])

    assert result == []


def test_find_by_context_types_returns_empty_list_for_invalid_json(
    mock_settings: Settings,
) -> None:
    recommendations_file = (
        Path(mock_settings.data_dir) / mock_settings.recommendations_file_name
